-- ─── Handle map_deleted events in recompute_user_profile() ──────────────
--
-- Problem:
--   The DELETE trigger on mindmaps inserts map_deleted events, but
--   recompute_user_profile() ignores them. Deleted maps still count
--   toward user_profiles totals, inflating the numbers.
--
-- Fix:
--   Add a 'map_deleted' case that mirrors the 'map_created' logic but
--   subtracts instead of adds. The same sub-map exclusion applies.
--
--   Daily activity tracking does NOT count map_deleted events — the
--   original activity (map creation) is still valid engagement data.
--
-- ────────────────────────────────────────────────────────────────────
  ...
  loop
    ...
    case v_event.event_type
      ...
      when 'map_deleted' then
        -- Mirror of map_created — subtract from totals
        v_is_sub_map := (v_event.event_data ->> 'is_sub_map')::boolean;
        if v_is_sub_map is not true and v_event.event_data ->> 'parent_map_id' is null then
