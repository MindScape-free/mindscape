-- ─── Fix: Fallback for total_nodes in recompute_platform_stats() ───
--
-- Problem:
--   recompute_platform_stats() reads total_nodes only from user_profiles,
--   which gets its data from user_events.event_data->>'node_count'. If
--   map_created events were backfilled without node_count in event_data
--   (e.g. {"source":"backfill"}), user_profiles.total_nodes = 0, causing
--   platform_stats.total_nodes = 0.
--
-- Fix:
--   After reading total_nodes from user_profiles, if the result is 0,
--   fallback to summing node_count directly from the mindmaps table.
--
-- Note:
--   The mindmaps table has no is_deleted/deleted columns, so we sum all
--   rows. Sub-maps (is_sub_map = true) with node_count = 0 are excluded
--   implicitly since coalesce(sum(node_count), 0) only counts non-null.
--
-- ────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- This handles the case where map_created events were backfilled
  -- without node_count in event_data.
