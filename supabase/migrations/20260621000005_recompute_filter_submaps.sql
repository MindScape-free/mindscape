-- ─── Filter sub-maps from the mindmaps fallback in recompute_platform_stats() ───
--
-- Problem:
--   The mindmaps fallback (when user_profiles.total_nodes = 0) sums ALL mindmaps
--   including sub-maps. The admin-sync endpoint explicitly excludes sub-maps
--   (is_sub_map = true or parent_map_id is not null) from its node count.
--   This inconsistency causes different node counts depending on which pipeline runs.
--
-- Fix:
--   Add WHERE clause to the fallback query to exclude sub-maps, matching
--   admin-sync's behavior: where (is_sub_map is not true or is_sub_map is null)
...
begin
  ...
  -- Excludes sub-maps to match admin-sync's behavior (only root maps
  -- contribute to the active node count).
