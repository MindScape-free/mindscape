...
export const FeedbackForm: React.FC<{ userId?: string, userName?: string, userEmail?: string }> = ({ userId, userName, userEmail }) => {
    ...
    const {
        ...
        defaultValues: {
            type: 'BUG', // Start with BUG to avoid undefined
            priority: 'MEDIUM',
            status: 'OPEN',
            title: '',
            description: '',
    ...
    const uploadFiles = async (feedbackId: string): Promise<string[]> => {
        ...
        for (const file of files) {
            const fileName = `${feedbackId}/${file.name.replace(/[^a-zA-Z0-9.-]/g, '_')}`;
                    ...
                    <form onSubmit={handleSubmit(onSubmit as any)}>
                        <AnimatePresence mode="wait">
                            ...
                            {step === 'DETAILS' && (
                                ...
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        <button type="button" onClick={() => setStep('CATEGORY')} className="p-3 rounded-2xl bg-white/5 text-zinc-500 hover:text-white transition-all duration-300">
                                                ...
                                                <div className="grid grid-cols-1 gap-2 mt-4">
                                                    {files.map((f, i) => (
                                                        <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-black/40 border border-white/5">
                                                            ...
                                                            <button type="button" onClick={(e) => {e.stopPropagation(); removeFile(i)}} className="p-1.5 hover:bg-destructive/10 rounded-lg transition-colors group">
                            ...
                            {step === 'REVIEW' && (
                                ...
                                >
                                    <div className="flex items-center gap-4 mb-4">
                                        <button type="button" onClick={() => setStep('DETAILS')} className="p-3 rounded-2xl bg-white/5 text-zinc-500 hover:text-white transition-all duration-300">
                                            <ChevronLeft className="w-5 h-5" />
                                        </button>
                                        <div className="space-y-0.5">
